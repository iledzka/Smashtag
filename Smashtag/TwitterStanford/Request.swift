...
//        }
    ...
    func performTwitterRequest(_ method: SLRequestMethod, handler: @escaping (PropertyList?) -> Void) {
        ...
        //changed url from URL? to String - new Twitter SDK
        let url = "\(Constants.twitterURLPrefix)\(self.requestType)\(jsonExtension)"
        //new Twitter SDK implementation
        if let twitterAccountId = TWTRTwitter.sharedInstance().sessionStore.session()?.userID {
            ...
            let request = client.urlRequest(withMethod: "GET", urlString: url, parameters: parameters, error: &clientError)
            
            client.sendTwitterRequest(request) { (httpResponse, jsonResponse, _) -> Void in
    ...
    //changed request from SLRequest to URLRequest
    /*func performTwitterSLRequest(_ request: URLRequest, handler: @escaping (PropertyList?) -> Void) {
